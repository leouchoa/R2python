...
\centering{\section*{Usefull examples}}
...
I wrote this document using Rstudio and Rmarkdown. So in order to load python within R the thing to do was to use `reticulate`. But  I've also installed all my python packages using anaconda because it handles compatability between libraries more efficiently. In that case you can load `reticulate` set it's path with `use_python` for it to load packages installed via anaconda (they're **not** the same).
...
# Multi argument iteration with `zip`
...
# Train-test Split and basic pre-process
